<%@page import="VAKS.run.GoogleCharts.GoogleCharts"%>
<%@page import="java.util.TreeMap"%>
<%@page import="java.util.Map"%>
<%@page import="java.util.HashMap"%>
<%@page import="VAKS.run.Controller.ManagerController"%>
...
<head>
<meta charset="UTF-8">
<title><%=LoginController.getLoggedInUsername()%> | Manager | VAKS</title>
	...
	<div class="admin-panel clearfix">
		<div class="slidebar">
			...
			<ul>
				...
				<li><a href="#User" onclick="showHideBar('User')">Manage
						User</a></li>
				<li><a href="#Calendar" onclick="showHideBar('Calendar')">Calendar</a></li>
				<li><a href="#ManageEvents"
					onclick="showHideBar('ManageEvents')">Manage Events</a></li>
									...
									<tbody>
										...
										<tr>
											...
											<td><input type="submit" Value="Delete"
												name="actionBtnValue" onClick="deleteConfirmUser(1)"></td>
							...
							<div class="AddUser-container " id="AddUser-container"
								...
								<div class="grid-item">
									<label>Role Type :</label> <select name="roleType"
										...
										<%
										ArrayList<String> roleType = AppWithDB.sqlSelectRoleType();
...
// 											System.err.println(id);
					...
					<div class="page-wrap">
						...
						<button class="open-button" onclick="openForm()">Create
							Appointment</button>
						​
						<div class="form-popup" id="myForm">
							<form action="createEvent" class="form-container" method="post">
								<h1>New Appointment</h1>
								​ <input type="radio" id="all" name="privacy" value="All"
									onclick="enableUserSelect()" required="required"> <label
									for="public">For All</label> <input type="radio" id="special"
									name="privacy" value="Special" onclick="enableUserSelect()">
								<label for="private">For Special</label><br> <label
									for="user"><b>User</b></label> <select name="forUser"
									id="forUser" required disabled="disabled">
									<%
									ResultSet r4 = null;
																							String Query4 = "Select user_id, first_name, last_name From User;";
																							Statement s4 = AppWithDB.connect().createStatement();
																							r4 = s4.executeQuery(Query4);
																							while (r4.next()) {
																								String user_id = r4.getString("user_id");
																								String first_name = r4.getString("first_name");
																								String last_name = r4.getString("last_name");
									%>
									...
									<%
																							...
																							s4.close();
									%>
								</select> <label for="title"><b>Title</b></label> <input type="text"
									name="title" id="title" required> <label for="date"><b>Date</b></label>
								<input type="date" name="date" required> <label
									for="startTime"><b>Start Time</b></label> <input type="time"
									name="startTime" required> <label for="endTime"><b>End
										Time</b></label> <input type="time" name="endTime" required> <label
									for="location"><b>Location</b></label> <input type="text"
									name="location" id="location">

								<button type="submit" class="btn">Create</button>
								<button type="button" class="btn cancel" onclick="closeForm()">Cancel
									and Close</button>
				...
				<div id="ManageEvents">
					...
					<div class="form-popup" id="myForm2">
							<form action="editEvent" class="form-container" method="post">
								<h1>Edit Appointment</h1>
								
								<input type="radio" id="all2" name="privacy" value="All" required="required"> 
								<label for="public">For All</label> 
								<input type="radio" id="special2" name="privacy" value="Special">
								<label for="private">For Special</label><br><b>User</b> 
								<input type="hidden" id="forUser2Hidden" name="for_user">
								<select name="forUser" id="forUser2" required disabled="disabled" >
									<%
									ResultSet r44 = null;
																							String Query44 = "Select user_id, first_name, last_name From User;";
							...
							<form action="showEvents" method="post">
								...
								<input type="date" name="search" id="myDate"  value= "<%=ManagerController.defaultDate%>" onchange="this.form.submit()">
								<br> <br>
								...
								<datalist id="brow">
													<%
													ResultSet r8 = null;
																																			String Query8 = "Select user_id, first_name, last_name From User;";
																																			Statement s8 = AppWithDB.connect().createStatement();
						...
						<div class="scrollit">
								...
								<table id="EventTable" class="table table-bordered">
									...
									<tbody>
										<%
										if (ManagerController.find.equals("basic")) {
																											
																										} else if (ManagerController.find.equals("byUser")) {
																											ResultSet r10 = null;
																											String Query10 = "Select DISTINCT event_id, event_name, from_date, to_date, from_time, to_time, location, user_id, first_name, last_name From Event  Natural Join User_Event Natural Join User Where for_user_id = '"
																											+ ManagerController.searchBy + "' AND user_id=for_user_id;";
																											Statement s10 = null;
																											...
																											while (r10.next()) {
																												...
																												String user_id = r10.getString("user_id");
																												String first_name = r10.getString("first_name");
																												String last_name = r10.getString("last_name");
										%>
...
<!-- 											<td><button onclick="openForm()">Edit1</button></td> -->
										...
										<%
																										...
																										r10.close();
																										s10.close();
																										} else if (ManagerController.find.equals("byDate")) {
																										
																										// To show Events for Users by Date
																										ResultSet r10 = null;
																										String Query10 = "Select DISTINCT event_id, event_name, from_date, to_date, from_time, to_time, location, user_id, first_name, last_name From Event  Natural Join User_Event Natural Join User Where from_date = '"
																												+ ManagerController.searchBy + "' AND user_id=for_user_id;";
																										Statement s10 = null;
																										s10 = AppWithDB.connect().createStatement();
																										r10 = s.executeQuery(Query10);
										...
										<%
																										...
																										r10.close();
																										s10.close();
																										// To show public (all) Events by Date
																										ResultSet r11 = null;
																										String Query11 = "Select DISTINCT event_id, event_name, from_date, to_date, from_time, to_time, location From Event  Natural Join User_Event Natural Join User Where from_date = '"+ ManagerController.searchBy +"' and for_user_id ISNULL;";
																										Statement s11 = null;
																										s11 = AppWithDB.connect().createStatement();
																										r11 = s.executeQuery(Query11);
																										while (r11.next()) {
																										String event_id = r11.getString("event_id");
																										String event_name = r11.getString("event_name");
																										String from_date = r11.getString("from_date");
																										String to_date = r11.getString("to_date");
																										String from_time = r11.getString("from_time");
																										String to_time = r11.getString("to_time");
																										String location = r11.getString("location");
										%>

										<tr>
											<td style="color:green"><%=event_id%></td>
											<td style="color:green"><%=event_name%></td>
											<td style="color:green"><%=from_date%></td>
											<td style="color:green"><%=to_date%></td>
											<td style="color:green"><%=from_time%></td>
											<td style="color:green"><%=to_time%></td>
				...
				<div id="Documents">
					...
					<form action="searchByUser_Document" method="post">
							
								<label>Search By User : </label>
								<input list="brow2" name="searchDocumentUser" onchange="this.form.submit()">
								<br> <br>
								 
								<datalist id="brow2">
								<option value="-- Public Documents --"></option>
								<option value="-- My Documents --"></option>
								<option value="-- All Documents --"></option>
								
													<%
																					ResultSet r81 = null;
																																																			String Query81 = "Select user_id, first_name, last_name From User;";
																																																			Statement s81 = AppWithDB.connect().createStatement();
																																																			r81 = s81.executeQuery(Query81);
																																																			while (r81.next()) {
																																																				String user_id = r81.getString("user_id");
																																																				String first_name = r81.getString("first_name");
																																																				String last_name = r81.getString("last_name");
																																																				if(!user_id.equals(LoginController.getLoggedInUserId())) {
																					%>
													<option value="<%=user_id%>"><%=first_name + " " + last_name.toUpperCase()%></option>
													<%
													}
						...
						<div class="scrollit">
							<form action="deleteDocument" method="post">
								<table id="DocumentTable" class="table table-bordered">
									...
									<tbody>
										<%
										if(ManagerController.documentSearch.equals("empty")){
																											
																											
																										}
																										else if (ManagerController.documentSearch.equals("-- Public Documents --")) {
																											
																											ResultSet r2 = null;
																											String Query2 = "Select document_id, document_name, document_language, Type.document_type, document_for_user FROM Document NATURAL JOIN Type Natural JOIN Document_Type Natural Join User_Document where User_Document.document_for_user='';";
										...
										%>
													...
													<tr>
														...
														<td style="color:green">-</td>
														<td ><a href="/downloadDocument/<%=document_id%>">Download</a></td>
														<td ><input type="submit" Value="Delete"
													...
													<%
																																...
																																} else if (ManagerController.documentSearch.equals("-- My Documents --")) {
													...
													<%
																																...
																																} else if (ManagerController.documentSearch.equals("-- All Documents --")) {
																																	
																																	ResultSet r2 = null;
																																	String Query2 = "Select document_id, document_name, document_language, Type.document_type, document_for_user FROM Document NATURAL JOIN Type Natural JOIN Document_Type Natural Join User_Document ORDER BY Type.document_type DESC;";
																																	Statement s2 = AppWithDB.connect().createStatement();
																																	r2 = s2.executeQuery(Query2);
																																	while (r2.next()) {
																																		String document_id = r2.getString("document_id");
																																		...
																																		if(document_type.equals("Public")) {
													%>
													...
													<%
													} else if (document_type.equals("For User")) {
													%>

													<tr>
														<td><%=document_id%></td>
														<td><%=document_name.split("<>")[0]%></td>
														<td><%=document_language%></td>
														<td><%=document_type%></td>
														<td><%=document_for_user + " - " + first_name + ", " + last_name.toUpperCase()%></td>
														<td><a href="/downloadDocument/<%=document_id%>">Download</a></td>
														<td><input type="submit" Value="Delete"
															name="actionBtnValue" onClick="deleteConfirmDocument()"></td>
													</tr>
													<%
													} else if (document_type.equals("Private")) {
																																			
																																			if(document_for_user.equals(LoginController.getLoggedInUserId())) {
													%>
													<tr>
														<td style="color:red"><%=document_id%></td>
														<td style="color:red"><%=document_name.split("<>")[0]%></td>
													...
													<%
																																	...
																																	ResultSet r222 = null;
																																	String Query222 = "Select document_id, document_name, document_language, Type.document_type, document_for_user FROM Document NATURAL JOIN Type Natural JOIN Document_Type Natural Join User_Document where User_Document.document_for_user='" + ManagerController.documentSearch + "';";
																																			
																																	Statement s222 = AppWithDB.connect().createStatement();
																																	r222 = s222.executeQuery(Query222);
																																	while (r222.next()) {
																																		String document_id = r222.getString("document_id");
																																		String document_name = r222.getString("document_name");
																																		String document_language = r222.getString("document_language");
																																		String document_type = r222.getString("document_type");
																																		String document_for_user = r222.getString("document_for_user");
																																		
																																		
																																		ResultSet r2222 = null;
																																		String Query2222 = "select first_name, last_name from User Where user_id='" + document_for_user + "';"; 
																																		Statement s2222 = AppWithDB.connect().createStatement();
																																		r2222 = s2222.executeQuery(Query2222);
																																		String first_name=null;
																																		String last_name=null;
																																		while (r2222.next()) {
																																			first_name = r2222.getString("first_name");
																																			last_name = r2222.getString("last_name");
																																		}
																																		r2222.close();
																																		s2222.close();
													%>
													...
													<tr>
														...
														<td><%=document_language%></td>
														<td><%=document_type%></td>
														<td><%=document_for_user + " - " + first_name + ", " + last_name%></td>
														<td><a href="/downloadDocument/<%=document_id%>">Download</a></td>
														<td><input type="submit" Value="Delete"
															name="actionBtnValue" onClick="deleteConfirmDocument()"></td>
													</tr>
						...
						<form action="uploadDocument" method="post" enctype="multipart/form-data">
							<div class="AddDocument-container " id="AddDocument-container" style="visibility: hidden">
								...
								<label>Type :</label> 
								<select name="Type" id="document_type" required onchange="enableDocumentForUser(this)">
									<%
									ResultSet r3 = null;
																							String Query3 = "Select document_type From Type;";
								...
								<input list="brow" name="document_for_user" id="document_for_user" disabled="disabled">
								<br> <br>
								 
								<datalist id="brow">
													<%
													ResultSet r9 = null;
																																			String Query9 = "Select user_id, first_name, last_name From User;";
																																			Statement s9 = AppWithDB.connect().createStatement();
																																			r9 = s9.executeQuery(Query9);
																																			while (r9.next()) {
																																				String user_id = r9.getString("user_id");
																																				String first_name = r9.getString("first_name");
																																				String last_name = r9.getString("last_name");
													%>
													<option value="<%=user_id%>"><%=first_name + " " + last_name.toUpperCase()%></option>
													<%
													}
				...
				<div id="Statistics">
					...
					<div class="page-wrap">
					<button onclick="showHideST('Residence_Permit')">Residence Permit</button>
					<button onclick="showHideST('German_Level')">German Level</button>
					<button onclick="showHideST('Home_Land')">Home Land</button>
					<button onclick="showHideST('Gender')">Gender</button>
					<button onclick="showHideST('Resident')">Resident</button>
					<br><br>
					
					<%
										Map<String, Integer> residencePermitAnswer = GoogleCharts.residenceCount();
																		Map<String, Integer> integrationCourseAnswer = GoogleCharts.integrationCourse();
																		Map<String, Integer> homeLandAnswer = GoogleCharts.homeLand();
																		HashMap<String, HashMap<String, String>> genderAnswer = GoogleCharts.gender();
																		Map<String, Integer> residentAnswer = GoogleCharts.resident();
										%>
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Task', 'Hours per Day'],
          <%
          for (Map.Entry<String, Integer> pair : residencePermitAnswer.entrySet()) {
        	  
        	%>
        	['<%=pair.getKey().toString().split(" ")[0]%>', <%=pair.getValue()%>],
			<%
          }
          %>
        ]);
        var options = {
          title: 'The Type of The Residence Permit of Users',
          is3D: true,
          chartArea:
          {
          left:20,
        ...
        var chart = new google.visualization.PieChart(document.getElementById('Residence_Permit'));
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Task', 'Hours per Day'],
          <%
          for (Map.Entry<String, Integer> pair : integrationCourseAnswer.entrySet()) {
        	  
        	%>
        	['<%=pair.getKey() + " German Level"%>', <%=pair.getValue()%>],
			<%
          }
          %>
        ]);
        ...
        var chart = new google.visualization.PieChart(document.getElementById('German_Level'));
        chart.draw(data, options);
      }
    </script>
    ...
    <script type="text/javascript">
      ...
      function drawRegionsMap() {
        var data = google.visualization.arrayToDataTable([
          ['Country',   'Person'],
          <%
          for (Map.Entry<String, Integer> pair : homeLandAnswer.entrySet()) {
//         	  System.err.println(pair.getKey() + " , " + pair.getValue());
        ...
        var chart = new google.visualization.GeoChart(document.getElementById('Home_Land'));
        chart.draw(data, options);
      };
    </script>
    ...
    function drawChart() {
      var data = google.visualization.arrayToDataTable([
        ...
        <%
        Object[] lands = genderAnswer.keySet().toArray();
        for(int i=0; i<lands.length; i++) {
        	
        	String femaleAndMale = genderAnswer.get(lands[i]).toString();
        	
        	String maleAnswer = femaleAndMale.split(", male=")[1].replace("}", "");
        	String femaleAnswer = femaleAndMale.replace("{female=", "").split(",")[0];
//         	System.err.println(lands[i] + ", " + maleAnswer + ", " + femaleAnswer);
        	
        
        	
        	%>
        	['<%=lands[i]%>', <%=maleAnswer%>, <%=femaleAnswer%> ],
			<%
      ...
      var options = {
        chart: {
          title: 'The ratio of males and females',
          subtitle: 'The ratio of males and females according to each country',
        },
      chartArea:
      {
      left:20,
      ...
      var chart = new google.charts.Bar(document.getElementById('Gender'));
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Task', 'Hours per Day'],
          <%
          for (Map.Entry<String, Integer> pair : residentAnswer.entrySet()) {
        ...
        var chart = new google.visualization.PieChart(document.getElementById('Resident'));
        chart.draw(data, options);
      }
    </script>
    
    <div id="Residence_Permit" style="width: 900px; height: 500px;" ></div>
    <div id="German_Level" style="width: 900px; height: 500px;" ></div>
    <div id="Home_Land" style="width: 900px; height: 500px;" ></div>
    <br>
    <div id="Gender" style="width: 900px; height: 500px;" ></div>
    <div id="Resident" style="width: 900px; height: 500px;" ></div>
	...
	<script>
		...
		function showHideBar(d) {
			...
			var divs = [ 'User', 'Calendar', 'Statistics', 'Documents',
					'ManageEvents' ];
		...
		function showHideST(d) {
			var onediv = document.getElementById(d);
// 			document.getElementById(onediv).style.display = 'block';
			
			var divs = [ 'Residence_Permit', 'German_Level', 'Home_Land', 'Gender', 'Resident'];
			for (var i = 0; i < divs.length; i++) {
				if (onediv != document.getElementById(divs[i])) {
